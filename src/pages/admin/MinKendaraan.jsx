...
export default function KendaraanCRUD() {
  ...
  useEffect(() => {
    ...
    const filtered = kendaraan.filter(k => {
      // Mencocokkan query dengan setiap kolom data
      return (
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      fetchKendaraan(); // Memuat ulang data untuk menampilkan data baru
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            fetchKendaraan(); // Memuat ulang data
  ...
  return (
    <div className="space-y-8 px-4 sm:px-8 md:px-16 pt-8 pb-12 bg-gray-50 min-h-screen">
      ...
      {/* Tabel dengan input pencarian */}
      <div className="bg-white rounded-2xl shadow p-6 max-w-4xl mx-auto mt-8">
        <div className="flex justify-between items-center mb-4">
            <h3 className="font-semibold text-gray-700 text-lg">Daftar Kendaraan</h3>
        ...
        <div className="overflow-x-auto border rounded-2xl">
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {/*Gunakan `filteredKendaraan` untuk me-render tabel */}
