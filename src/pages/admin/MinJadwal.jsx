...
export default function JadwalCRUD() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      setFilteredJadwals(jadwalData); // Inisialisasi data filter
      setRutes(ruteData);
    } catch {
  ...
  //  useEffect untuk memfilter jadwal secara real-time
  useEffect(() => {
    const lowercasedQuery = searchQuery.toLowerCase();
    const filtered = jadwals.filter(j => {
      const ruteName = j.rute?.nama_rute || "";
      // Mencocokkan query dengan tanggal, waktu, atau nama rute
      return (
      ...
      <div className="bg-white rounded-2xl shadow p-6 max-w-5xl mx-auto mt-8">
        ...
        <div className="overflow-x-auto border rounded-2xl">
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {/* Gunakan `filteredJadwals` untuk me-render tabel */}
